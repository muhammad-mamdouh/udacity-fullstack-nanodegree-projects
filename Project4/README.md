...
```ssh grader@35.159.25.27 -p 2200 -i ~/.ssh/provided_grader_key```
...
On your local machine terminal run ```ssh-keygen``` 
...
```sudo ufw allow 2200/tcp -- allow all ssh request over 2200 port```

```sudo ufw allow 123/tcp -- allow all ntp request over 123 port```
...
after that you have to configure supervisor by opening 

```sudo nano /etc/supervisor/conf.d/CHOSEN_APP_NAME.conf```
